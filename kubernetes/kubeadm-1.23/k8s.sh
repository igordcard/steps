...
#kubeadm init --pod-network-cidr=10.210.0.0/16 # --v=5 # replace with correct cp node ip address
#  for nodus use:
kubeadm init --kubernetes-version=1.23.6 --pod-network-cidr=10.151.142.0/18 #--apiserver-advertise-address 10.0.0.5 --control-plane-endpoint 10.0.0.5 # --v=5 # replace with correct cp node ip address
#  for flannel use:
kubeadm init --kubernetes-version=1.23.6 --pod-network-cidr=10.244.0.0/16
# for calico use:
kubeadm init --kubernetes-version=1.23.6 --pod-network-cidr=192.168.0.0/16
...
# wait for node to become Ready and make sure it has a master role:
